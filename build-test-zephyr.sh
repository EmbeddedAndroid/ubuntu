...
echo "Source Mirror Update Started $(date +"%T")" > /mnt/artifacts/$ZEPHYR_BUILD_ID/results.txt
git remote update
echo "Source Mirror Update Finished $(date +"%T")" >> /mnt/artifacts/$ZEPHYR_BUILD_ID/results.txt

# Clone and Build
cd /root
echo "Clone Source Started $(date +"%T")" >> /mnt/artifacts/$ZEPHYR_BUILD_ID/results.txt
git clone --reference /srv/mirrors/zephyr-mirror $ZEPHYR_SOURCE zephyr-project
echo "Clone Source Finished $(date +"%T")" >> /mnt/artifacts/$ZEPHYR_BUILD_ID/results.txt
cd zephyr-project
...
echo "Build Started $(date +"%T")" >> /mnt/artifacts/$ZEPHYR_BUILD_ID/results.txt
sanitycheck --platform $ZEPHYR_PLATFORM --build-only --outdir $ZEPHYR_OUTDIR --enable-slow --no-clean
echo "Build Finished $(date +"%T")" >> /mnt/artifacts/$ZEPHYR_BUILD_ID/results.txt
...
echo "Running Tests Finished $(date +"%T")" >> /mnt/artifacts/$ZEPHYR_BUILD_ID/results.txt
cd /cat results.txt >> /mnt/artifacts/$ZEPHYR_BUILD_ID/results.txt
